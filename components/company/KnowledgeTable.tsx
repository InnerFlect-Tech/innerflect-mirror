...
/**
 * The Practical view: the same knowledge as a table you can scan. Drift keeps
 * the top of the surface, because the difference between what is documented and
 * what actually happens is the thing Mirror exists to reveal.
 */
...
export function KnowledgeTable({ objects }: { objects: KnowledgeObject[] }) {
  ...
  return (
    <div className="knowledge-surface">
      ...
      <table className="knowledge-table">
        <caption className="sr-only">Knowledge objects with trust, freshness, owner, usage and conflicts.</caption>
        <thead>
          <tr>
            <th scope="col">Object</th><th scope="col">Kind</th><th scope="col">Trust</th>
            <th scope="col">Last verified</th><th scope="col">Owner</th><th scope="col">Used by</th>
          </tr>
        </thead>
        <tbody>
          {objects.map((o) => (
            <tr key={o.id}>
              ...
              <td>
                ...
                <em className="trust-bar" aria-hidden="true"><i style={{ width: `${o.trust}%`, background: trustTone(o.trust) }} /></em>
              </td>
              <td data-freshness={o.freshness}>{o.lastVerified}<small>{FRESHNESS_LABEL[o.freshness]}</small></td>
